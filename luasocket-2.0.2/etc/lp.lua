...
--[[
RFC 1179
5.3 03 - Send queue_obj.lua state (short)

      +----+-------+----+------+----+
      | 03 | Queue | SP | List | LF |
      +----+-------+----+------+----+
      Command code - 3
      Operand 1 - Printer queue_obj.lua name
      Other operands - User names or job numbers

   If the user names or job numbers or both are supplied then only those
   jobs for those users or with those numbers will be sent.

   The response is an ASCII stream which describes the printer queue_obj.lua.
   The stream continues until the connection closes.  Ends of lines are
   indicated with ASCII LF control characters.  The lines may also
   contain ASCII HT control characters.

5.4 04 - Send queue_obj.lua state (long)
...
--       +----+-------+----+
--       Command code - 2
--       Operand - Printer queue_obj.lua name
--
...
--
-- lp.query({host=,queue_obj.lua=printer|'*', format='l'|'s', list=})
--
